...
def getTrue():
    ...
    # 抓包设置
    find_c = {'0':'tcp', '1':'udp', '2':'port 80', '3':'port 443', '4':'port 20 or port 21', '5':'port 25', '6':'port 110', '7':'port 53', '8':'port 22', '9':'port 23'}
    factor = None
    c = [[0,c_tcp.get()] , [1,c_udp.get()], [2,c_http.get()], [3,c_https.get()], [4,c_ftp.get()], [5,c_smtp.get()], [6,c_pop3.get()], [7,c_dns.get()], [8,c_ssh.get()], [9,c_telnet.get()]]
    ...
    typenum.append( len(bagcontent) - len(bagcontent[TCP]) - len(bagcontent[UDP]) - len(bagcontent[ICMP]) )
    # 获取数据包信息
...
def a_window(info):
    ...
    top2 = Toplevel()
    top2.title('详细信息')
    show_msg(top2, info)    # 递归调用，显示包中所有的信息

    h = 0
    l = 0

def show_msg(top2, info):
    global h
    global l

    Label(top2, text = '###[ ' + info.name + ' ]###').grid(stick = W, row = h, column = l, padx = 8)
    h, l = add(h, l)
    dict_one = info.fields
    for item in dict_one:
        # isinstance(dict_one[item],DNSQR)判断是否是类的实例
        if ((type(dict_one[item]) is list) and (item != 'options') and type(dict_one[item][0]) is not int and (type(dict_one[item][0]) is not str)) or isinstance(dict_one[item],DNSQR) or isinstance(dict_one[item],DNSRR):
            show_msg(top2, dict_one[item][0])
        else:
            Label(top2, text = item + ': ' + repr(dict_one[item]), wraplength = 400, justify = 'left').grid(stick = W, row = h, column = l, padx = 8)
            h, l = add(h, l)

    Label(top2, text = '').grid(stick = W, row = h, column = l, padx = 8)  # 空行
    h, l = add(h, l)
...
def add(h, l):  # 这个函数是为了控制行列书以确保能够在窗口中全部显示
    h += 1
    if h >= 26:
        h = 0
        l += 1
...
if __name__ == "__main__":
    ...
    # 空行调整格式
    inb = Label(top, height=2, text = "(复选框可捕获指定协议的数据包（可都选），若都不选则捕获所有可捕获数据包)")
    ...
    ftp_b = Checkbutton(top, text = "FTP", variable = c_ftp)
    ftp_b.grid(row = 4, column = 3, sticky = W)
    smtp_b = Checkbutton(top, text = "SMTP", variable = c_smtp)
    smtp_b.grid(row = 4, column = 4, sticky = W)
    pop3_b = Checkbutton(top, text = "POP3", variable = c_pop3)
    pop3_b.grid(row = 5, column = 1, sticky = E)
    dns_b = Checkbutton(top, text = "DNS", variable = c_dns)
    dns_b.grid(row = 5, column = 2, sticky = W)
    ssh_b = Checkbutton(top, text = "SSH", variable = c_ssh)
    ssh_b.grid(row = 5, column = 3, sticky = W)
    telnet_b = Checkbutton(top, text = "Telnet", variable = c_telnet)
    telnet_b.grid(row = 5, column = 4, sticky = W)

...
if __name__ == "__main__":
    ...
    # 开始抓包按钮,command触发事件
    begin = Button(top, text = "开始捕获数据包", activebackground = 'gray')
    ...
    print(pattern_SELECT)
    # 最终确认信息，使用一个按钮，不勾选
    # var = IntVar()
    # checkbutton = Checkbutton(top, text='最终确认信息', variable=var)
    # checkbutton.grid(columnspan=2, sticky=E)
    ARP_def = Button(top, text = "  确  认  ", activebackground = 'gray')
    ARP_def.grid(row = 10, column = 1, sticky = W)


    # 调用子函数ARP_spoofing
    if ??? = True:
        ARP_spoofing(gateway_IP, target_IP, interfacetype, pattern_SELECT)

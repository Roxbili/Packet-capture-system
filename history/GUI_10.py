    ...
    for i, tof in c:
        if tof:
            if factor == None:
                ...
                factor = 'or' + find_c[str(i)]

    bagcontent = get_bag(bagnum, factor) # 子函数名get_bag
...
def a_window(info):
    ...
    if 'Ether' in info:
        ...
        dict_Ether = info[Ether].fields
        for item in dict_Ether:
            Label(top2, text = item + ': ' + str(dict_Ether[item])).grid(stick=W)
        Label(top2, text = '').grid()  # 空行

    if 'IP' in info:
        Label(top2, text = '###[ IP ]###').grid(stick=W)
        dict_IP = info[IP].fields
        for item in dict_IP:
            Label(top2, text = item + ': ' + str(dict_IP[item])).grid(stick=W)
        Label(top2, text = '').grid()  # 空行

    if 'TCP' in info:
        Label(top2, text = '###[ TCP ]###').grid(stick=W)
        dict_TCP = info[TCP].fields
        for item in dict_TCP:
            Label(top2, text = item + ': ' + str(dict_TCP[item])).grid(stick=W)
        Label(top2, text = '').grid()  # 空行
        ...
        if 'HTTP' in info:
            ...
            if 'HTTP Request' in info:
                ...
                dict_httpRq = info[HTTPRequest].fields
                for item in dict_httpRq:
                    Label(top2, text = item + ': ' + str(dict_httpRq[item])).grid(stick=W)
                Label(top2, text = '').grid()  # 空行

            elif 'HTTP Response' in info:
                Label(top2, text = '###[ HTTP Response ]###').grid(stick=W)
                dict_httpRp = info[HTTPResponse].fields
                for item in dict_httpRp:
                    Label(top2, text = item + ': ' + str(dict_httpRp[item])).grid(stick=W)
                Label(top2, text = '').grid()  # 空行
                
    if 'UDP' in info:
        Label(top2, text = '###[ UDP ]###').grid(stick=W)
        dict_UDP = info[UDP].fields
        for item in dict_UDP:
            Label(top2, text = item + ': ' + str(dict_UDP[item])).grid(stick=W)
        Label(top2, text = '').grid()  # 空行

    if 'DNS' in info:
        Label(top2, text = '###[ DNS ]###').grid(stick=W)
        dict_DNS = info[DNS].fields
        for item in dict_DNS:
            Label(top2, text = item + ': ' + str(dict_DNS[item])).grid(stick=W)
        Label(top2, text = '').grid()  # 空行

    if 'Raw' in info:
        Label(top2, text = '###[ Raw ]###').grid(stick=W)
        dict_Raw = info[Raw].fields
        for item in dict_Raw:
            Label(top2, text = item + ': ' + dict_Raw[item]).grid(stick=W)
            # print(item + str(dict_Raw[item]))
            # pass
        Label(top2, text = '').grid()  # 空行
...
if __name__ == "__main__":
    ...
    c_http = BooleanVar()
    c_https = BooleanVar()
    c = [(0,c_tcp) , (1,c_udp), (2,c_http), (3,c_https)]
